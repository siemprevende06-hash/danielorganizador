...
interface AntiVisionSection {
  id: string;
  name: string;
  rows: number;
  cards: AntiVisionCard[];
}
...
function makeCards(rows: number): AntiVisionCard[] {
...
export default function AntiVisionPage() {
  const { data: sections, setData: setSections } = useTextSection<AntiVisionSection[]>(
    "antivision-data",
    []
  );
  ...
  const updateSection = (id: string, patch: Partial<AntiVisionSection>) => {
    ...
    <div className="min-h-screen bg-background p-4 md:p-6 pt-20 pb-24">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Anti-Visión</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Define tu anti-visión con secciones de imágenes
            </p>
          </div>
        ...
        {sections.length === 0 && (
          <Card className="p-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              No hay secciones aún. Crea una para empezar a definir tu anti-visión.
            </p>
