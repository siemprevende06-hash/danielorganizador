...
export default function Finance() {
  ...
  const handleRevertTransaction = async () => {
    ...
    const wallet = wallets.find(w => w.id === transactionToRevert.walletId);
    if (wallet) {
      const delta = transactionToRevert.type === 'income' ? -transactionToRevert.amount : transactionToRevert.amount;
      await updateWalletBalance(wallet.id, wallet.balance + delta);
    }
    await deleteTransaction(transactionToRevert.id);
    setTransactionToRevert(null);
    setIsRevertDialogOpen(false);
  ...
  const transactionColumns = useMemo(
    () => getTransactionColumns(wallets, transactionCategories, exchangeRate, (t) => {
      setTransactionToRevert(t);
      setIsRevertDialogOpen(true);
    }),
    [wallets, exchangeRate]
  );
  ...
  const loanColumns = useMemo(
    () => getLoanColumns(exchangeRate, openLoanPaymentDialog),
    [exchangeRate]
  );

  const handleOpenTransactionDialog = () => {
    transactionForm.reset({ description: '', amount: 0, currency: 'CUP', walletId: '', categoryId: '', type: 'expense', date: new Date() } as any);
    setIsTransactionDialogOpen(true);
  };

  const openWalletDialog = (w: Wallet) => {
    setWalletToEdit(w);
    walletForm.reset({ name: w.name, balance: w.balance, currency: 'CUP' });
    setIsWalletDialogOpen(true);
  };

  const toUsd = (amount: number, currency: 'USD' | 'CUP') =>
    currency === 'CUP' ? amount / exchangeRate : amount;

  const onTransactionSubmit = async (values: z.infer<typeof transactionSchema>) => {
    const amountUsd = toUsd(values.amount, values.currency);
    await addTransaction({
    ...
    if (wallet) {
      const delta = values.type === 'income' ? amountUsd : -amountUsd;
      await updateWalletBalance(wallet.id, wallet.balance + delta);
    }
    transactionForm.reset();
    setIsTransactionDialogOpen(false);
  ...
  const onTransferSubmit = async (values: z.infer<typeof transferSchema>) => {
    const amountUsd = toUsd(values.amount, values.currency);
    const from = wallets.find(w => w.id === values.fromWalletId);
    const to = wallets.find(w => w.id === values.toWalletId);
    if (!from || !to) return;
    await updateWalletBalance(from.id, from.balance - amountUsd);
    await updateWalletBalance(to.id, to.balance + amountUsd);
    transferForm.reset();
    setIsTransferDialogOpen(false);
  ...
  const onWalletSubmit = async (values: z.infer<typeof walletSchema>) => {
    if (walletToEdit) {
      await updateWallet(walletToEdit.id, { name: values.name, balance: values.balance });
    }
    setWalletToEdit(null);
    walletForm.reset();
    setIsWalletDialogOpen(false);
  ...
  const onLoanSubmit = async (values: z.infer<typeof loanSchema>) => {
    const amountUsd = toUsd(values.amount, values.currency);
    await addLoan({
  ...
  const onLoanPaymentSubmit = async (values: z.infer<typeof loanPaymentSchema>) => {
    if (!loanToPay) return;
    const amountUsd = toUsd(values.amount, values.currency);
    const newPaid = Math.min(loanToPay.totalAmount, loanToPay.paidAmount + amountUsd);
    const status: 'outstanding' | 'paid' = newPaid >= loanToPay.totalAmount ? 'paid' : 'outstanding';
    await updateLoan(loanToPay.id, { paidAmount: newPaid, status });
    setLoanToPay(null);
    loanPaymentForm.reset();
    setIsLoanPaymentDialogOpen(false);
  ...
  const onDebtSubmit = async (values: z.infer<typeof debtSchema>) => {
    const amountUsd = toUsd(values.amount, values.currency);
    await addDebt({
      person: values.person,
      description: values.description,
  ...
  const onDebtPaymentSubmit = async (values: z.infer<typeof debtPaymentSchema>) => {
    if (!debtToPay) return;
    const amountUsd = toUsd(values.amount, values.currency);
    const newPaid = Math.min(debtToPay.totalAmount, debtToPay.paidAmount + amountUsd);
    const status: 'outstanding' | 'paid' = newPaid >= debtToPay.totalAmount ? 'paid' : 'outstanding';
    await updateDebt(debtToPay.id, { paidAmount: newPaid, status });
    setDebtToPay(null);
    debtPaymentForm.reset();
    setIsDebtPaymentDialogOpen(false);
  ...
  const onBagSubmit = async (values: z.infer<typeof bagSchema>) => {
    if (editingBag) {
      await updateDistributionBag(editingBag.id, values);
    } else {
      await addDistributionBag({
        name: values.name,
  ...
  const handleDeleteBag = async () => {
    if (!bagToDelete) return;
    await deleteDistributionBag(bagToDelete.id);
    setBagToDelete(null);
  };

  const handleConfirmDistribution = async () => {
    setTransactions(prev => prev.map(t => (
      t.type === 'income' && !t.distributed && t.categoryId !== 'cat-transfer'
        ? { ...t, distributed: true }
        : t
    )));
    setIsDistributeIncomeDialogOpen(false);
    toast({ title: 'Ingresos distribuidos' });
  };
