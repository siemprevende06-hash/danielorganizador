...
export default function Focus() {
  ...
  // Load available tasks - ALL pending tasks, not just today's
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {/* Tasks assigned to current block */}
            {currentBlock && (() => {
              const currentBlockTasks = availableTasks.filter(t => t.routine_block_id === currentBlock.id);
              if (currentBlockTasks.length > 0) {
                return (
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                      Tareas del bloque actual ({currentBlock.title})
                    </p>
                    <div className="grid gap-1.5">
                      {currentBlockTasks.map(task => (
                        <button
                          key={task.id}
                          onClick={() => { setSelectedTaskId(task.id); setCustomTaskTitle(''); }}
                        ...
                        >
                          <span className="flex items-center gap-2">
                            {task.priority === 'high' && <AlertTriangle className="w-3 h-3 text-destructive flex-shrink-0" />}
                            {task.title}
                            <Badge variant="outline" className="text-[10px] ml-auto">{getSourceBadge(task.source).name}</Badge>
                          </span>
                        </button>
                      ))}
                    </div>
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">
                {currentBlock ? 'Otras tareas pendientes' : 'Tareas pendientes'}
              </p>
              <Select value={selectedTaskId || "none"} onValueChange={(v) => {
                ...
                <SelectContent>
                  <SelectItem value="none">-- Escribir manual --</SelectItem>
                  {availableTasks
                    .filter(t => !currentBlock || t.routine_block_id !== currentBlock.id)
                    .map(task => (
            ...
            <div className="flex gap-2">
              <Input 
                ...
                onChange={(e) => { setCustomTaskTitle(e.target.value); setSelectedTaskId(""); }}
                disabled={!!selectedTaskId}
                onKeyDown={(e) => e.key === 'Enter' && !isRunning && handleStartFocus()}
              />
            </div>

            {focusedTask && (
              <div className="p-3 bg-primary/10 rounded-lg border border-primary/30">
                <p className="text-xs text-muted-foreground">Enfocado en:</p>
                <p className="font-semibold text-sm">{focusedTask.title}</p>
                {focusedTask.area && (
