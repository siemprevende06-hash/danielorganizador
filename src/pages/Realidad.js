...
export default function Realidad() {
    const { data: sections, setData: setSections, loading, saving, saveNow } = useTextSection("realidad-data", []);
        ...
        valid.forEach((url) => {
            getImageBlob(url).then((existing) => {
                ...
                fetch(url, { mode: "cors" }).then((res) => {
                    if (res.ok)
                        res.blob().then((blob) => storeImageBlob(url, blob));
                }).catch(() => { });
            });
        });
    }, [sections]);
