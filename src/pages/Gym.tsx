...
import { useWorkoutTracking, TrainingType, computeSessionSummary } from "@/hooks/useWorkoutTracking";
import { Check, ChevronRight, Minus, Plus, Timer, Dumbbell, Play, SkipForward, Trophy, ChevronDown, ChevronUp, Box, CalendarDays, Scale, TrendingUp } from "lucide-react";
...
import { WeeklyProgressCard } from "@/components/gym/WeeklyProgressCard";
import { StrengthPanel } from "@/components/gym/StrengthPanel";
import { MonthlyAttendanceCalendar } from "@/components/gym/MonthlyAttendanceCalendar";
import { MeasurementTracker } from "@/components/gym/MeasurementTracker";
import { ProgressiveOverloadPanel } from "@/components/gym/ProgressiveOverloadPanel";
import { WorkoutPlanner } from "@/components/gym/WorkoutPlanner";
...
const parseTargetUpper = (targetReps: string) => {
  const m = targetReps.match(/(\d+)/g);
  return m ? parseInt(m[m.length - 1]) : 8;
};
...
export default function Gym() {
  const {
    routine, allRoutines, exercises, logs, isLoading,
    createRoutine, addExercise, removeExercise, logWorkout, selectRoutine,
    getTodayWorkout, getExercisesByDay, reload, DAY_NAMES, loadAllRoutines, sessions,
    startSession, finishSession, syncWorkoutDuration, getExerciseHistory, getLastLog,
    getSessionPrNames
  } = useWorkoutTracking();

  const [trainingType, setTrainingType] = useState<TrainingType>('gimnasio');
  ...
  const todayWorkout = routine
    ? plannedDay
      ? { dayName: DAY_LABELS[plannedDay] || plannedDay, isWorkoutDay: true, exercises: getExercisesByDay(plannedDay) }
      : getTodayWorkout()
    : null;
  const todayExercises = todayWorkout?.exercises || [];
  const sessionSummaries = sessions.map(s => computeSessionSummary(s, exercises));
  const sessionTotals = sessionSummaries.reduce((acc, s) => ({
    minutes: acc.minutes + (s.duration_minutes || 0),
    volume: acc.volume + s.totalVolumeKg,
    sets: acc.sets + s.setCount
      ...
      setSessionSets(
        todayExercises.map(ex =>
          Array.from({
            ...
            weight: trainingType === 'gimnasio' ? (getLastLog(ex.id)?.weight_kg || 0) : 0,
  ...
  }, [todayExercises.length, trainingType]);
  ...
  const hintFor = (ex: { id: string; target_reps: string }) => {
    const last = getLastLog(ex.id);
    if (!last || !last.weight_kg || Number(last.weight_kg) <= 0) return null;
    const total = (last.reps_per_set || []).reduce((a: number, b: number) => a + (Number(b) || 0), 0);
    const upper = parseTargetUpper(ex.target_reps);
    const avg = Number(last.weight_kg);
    if (total >= upper) return `Ultima marca ${avg} kg x ${total} reps. Progresion: sube a ${avg + 2.5} kg`;
    return `Ultima marca: ${avg} kg x ${total} reps (objetivo ${upper})`;
  };
  ...
  const handleNext = async () => {
    ...
    if (allSetsComplete) {
      if (currentExerciseObj) {
        const weights = currentExSets.map(s => s.weight);
        const avgWeight = trainingType === 'gimnasio'
        ...
        const res = await logWorkout(
          ...
          trainingType === 'gimnasio' ? weights : undefined
        );
        if (res.data?.is_pr) setPrNames(prev => [...prev, currentExerciseObj.name]);
      }
      if (activeExercise < todayExercises.length - 1) {
        ...
        const totalSecs = sessionStartRef.current ? Math.floor((Date.now() - sessionStartRef.current) / 1000) : elapsedSeconds;
        const totalMins = Math.max(1, Math.round(totalSecs / 60));
        if (activeSessionId) {
          finishSession(activeSessionId, totalMins);
          syncWorkoutDuration(totalMins);
        }
  ...
  const handleCreateRoutine = async () => {
    ...
    await createRoutine(newRoutineName, selectedDays, trainingType);
    setSetupOpen(false);
    setNewRoutineName("");
    setSelectedDays({});
  ...
  const handleSelectRoutine = async (routineId: string) => {
    ...
    toast({ title: "Rutina seleccionada" });
  };
  ...
  const beginWorkout = () => {
    ...
    startSession(routine!.id, trainingType).then(res => {
      if (res.data) setActiveSessionId(res.data.id);
    });
  };

  const handleStartPlanned = (dayKey: string) => {
    setPlannedDay(dayKey);
    sessionStartRef.current = Date.now();
    setElapsedSeconds(0);
    setPrNames([]);
    setWorkoutStarted(true);
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2 bg-muted p-1 rounded-lg">
            <button
              onClick={() => {
                setTrainingType('gimnasio');
                resetWorkout();
              }}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all",
                trainingType === 'gimnasio' ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
            ...
            <button
              onClick={() => {
                setTrainingType('calistenia');
                resetWorkout();
              }}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all",
                trainingType === 'calistenia' ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
        ...
        {!routine ? (
          /* No active routine: show all routines list + create */
          <div className="space-y-6">
            <div className="text-center space-y-3">
              {trainingType === 'gimnasio' ? (
                <Dumbbell className="h-12 w-12 mx-auto text-primary" />
              ) : (
                <Box className="h-12 w-12 mx-auto text-teal-500" />
              )}
              <h1 className="text-2xl font-bold">
                {trainingType === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}
              </h1>
              <p className="text-muted-foreground">
                {allRoutines.length === 0
                  ? "No tienes rutinas creadas. ¡Crea tu primera rutina!"
                  : "Selecciona una rutina para empezar a entrenar"}
              </p>
            </div>
            ...
            {allRoutines.length > 0 && (
              <div className="space-y-3">
                ...
                {allRoutines.map(r => {
                  const daysActive = Object.entries(r.workout_days || {})
                    .filter(([, v]) => v)
                    .map(([k]) => DAY_LABELS[k]?.slice(0, 3))
                    .join(", ");
                  return (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant={r.tipo === 'gimnasio' ? 'default' : 'secondary'} className="text-[10px]">
                              {r.tipo === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}
                            </Badge>
                            {r.is_active && (
                              <Badge variant="outline" className="text-[10px] text-green-600 border-green-400">Activa</Badge>
                            )}
                          </div>
                          <p className="text-xs text-muted-foreground">{daysActive || "Sin días"}</p>
                        </div>
                        <Button size="sm" onClick={() => handleSelectRoutine(r.id)} disabled={r.is_active}>
                          {r.is_active ? "Seleccionada" : "Seleccionar"}
                        </Button>
                      </div>
                    </Card>
                  );
              ...
              <Dialog open={setupOpen} onOpenChange={setSetupOpen}>
                <DialogTrigger asChild>
                  <Button size="lg" className="gap-2">
                    <Plus className="h-5 w-5" /> Crear Rutina de {
                      trainingType === 'gimnasio' ? 'Gimnasio' : 'Calistenia'
                    }
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Nueva Rutina de {trainingType === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}</DialogTitle>
                  </DialogHeader>
          ...
          /* Routine exists: show workout UI */
          <>
            {workoutComplete ? (
              <div className="max-w-lg mx-auto text-center space-y-6">
                ...
                <p className="text-sm font-semibold text-primary mt-2 flex items-center justify-center gap-1">
                  <Timer className="h-4 w-4" />Duracion: {formatElapsed(elapsedSeconds)}
                </p>
                {prNames.length > 0 && (
                  <div className="space-y-1.5">
                    <p className="text-xs uppercase tracking-widest text-muted-foreground">Nuevos records (PR)</p>
                    <div className="flex flex-wrap justify-center gap-1.5">
                      {prNames.map(n => (
                        <Badge key={n} className="bg-amber-500/20 text-amber-600 border-amber-500/30 text-[10px]">
                          {n} PR
                        </Badge>
                      ))}
                    </div>
              ...
              <>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={trainingType === 'gimnasio' ? 'default' : 'secondary'}>
                        {trainingType === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}
                      </Badge>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {todayWorkout?.dayName}{plannedDay ? ` · ${todayExercises.length} ejercicios` : ""} · {routine.name}
                    </p>
                  </div>
                ...
                <Tabs defaultValue="workout">
                  <TabsList className="grid grid-cols-8">
                    ...
                    <TabsTrigger value="planificacion">Planificar</TabsTrigger>
                  ...
                  <TabsContent value="planificacion" className="mt-3">
                    <WorkoutPlanner
                      routine={routine}
                      exercises={exercises}
                      getExercisesByDay={getExercisesByDay}
                      onStartWorkout={handleStartPlanned}
                  ...
                  <TabsContent value="medidas" className="mt-3">
                    <MeasurementTracker />
                  </TabsContent>
                  ...
                  <TabsContent value="progreso" className="mt-3">
                    <div className="space-y-4">
                      <ProgressiveOverloadPanel exercises={exercises} logs={logs} />
                      <Card className="p-4">
                          ...
                          exercises.map(ex => {
                            ...
                            return (
                              <div key={ex.id} className="mb-4 last:mb-0">
                                ...
                                <button
                                  className="text-[10px] text-muted-foreground hover:text-foreground underline underline-offset-2 mb-1"
                                  onClick={() => setExpandedChart(expandedChart === ex.id ? null : ex.id)}
                                >
                                  {expandedChart === ex.id ? 'Ocultar evolucion' : 'Ver evolucion'}
                                </button>
                                {expandedChart === ex.id && (
                                  <div className="mb-2">
                                    <ExerciseProgressChart points={getExerciseHistory(ex.id)} />
                                  </div>
                                )}
                                  ...
                                  <table className="w-full text-[10px]">
                                    <thead>
                                      <tr className="text-muted-foreground border-b">
                                        ...
                                        {trainingType === 'gimnasio' && <th className="text-center py-1 px-2">Peso</th>}
                                          ...
                                          {trainingType === 'gimnasio' && (
                                            editingLog === log.id ? (
                                              ...
                                              <td className="text-center py-1 px-2 font-medium">
                                                {log.weight_kg || '—'} kg{' '}
                                                {log.is_pr && <Badge className="ml-1 bg-amber-500/20 text-amber-600 border-amber-500/30 text-[9px]">PR</Badge>}
                                              </td>
                                            )
                                          )}
                                          {editingLog === log.id ? (
                                            <>
                                              ...
                                              <td className="text-center py-1 px-2">
                                                <div className="flex gap-0.5 justify-center">
                                                  ...
                                                  <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => deleteLogEntry(log.id)}>
                                            ...
                                            <>
                                              <td className="text-center py-1 px-2 cursor-pointer" onClick={() => {
                                                setEditingLog(log.id);
                                                setEditWeight(String(log.weight_kg || ''));
                                                setEditReps((log.reps_per_set || []).join(','));
                                              }}>
                  ...
                  <TabsContent value="historial" className="mt-3">
                    <div className="space-y-3">
                      {sessionSummaries.length === 0 ? (
                        <Card className="p-4 text-center"><p className="text-xs text-muted-foreground">Aun no hay entrenamientos registrados. Completa tu primera sesion!</p></Card>
                      ) : (
                        <>
                          <Card className="p-4">
                            <div className="grid grid-cols-3 gap-2 text-center">
                              <div><p className="text-lg font-bold text-primary">{sessionSummaries.length}</p><p className="text-[10px] text-muted-foreground">sesiones</p></div>
                              <div><p className="text-lg font-bold text-primary">{sessionTotals.minutes}</p><p className="text-[10px] text-muted-foreground">min totales</p></div>
                              <div><p className="text-lg font-bold text-primary">{Math.round(sessionTotals.volume)}</p><p className="text-[10px] text-muted-foreground">kg volumen</p></div>
                            </div>
                          </Card>
                          {sessionSummaries.map(sm => {
                            const start = new Date(sm.started_at);
                            const expanded = expandedSession === sm.id;
                            return (
                              <Card key={sm.id} className="p-4">
                                <button className="flex items-center justify-between w-full text-left" onClick={() => setExpandedSession(expanded ? null : sm.id)}>
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <span className="font-semibold capitalize">
                                        {start.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}
                                      </span>
                                      <Badge variant={sm.tipo === 'gimnasio' ? 'default' : 'secondary'} className="text-[10px]">
                                        {sm.tipo === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}
                                      </Badge>
                                    </div>
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                      {sm.exerciseCount} ejercicios · {sm.setCount} series · {Math.round(sm.totalVolumeKg)} kg volumen
                                    </p>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                                      <Timer className="h-3 w-3" />{sm.duration_minutes ? `${sm.duration_minutes} min` : '—'}
                                    </span>
                                ...
                                {expanded && (
                                  <div className="mt-3 space-y-2">
                                    {sm.exercises.map((ex, i) => (
                                      <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded bg-muted/50">
                                        <span className="text-sm font-medium">{ex.name}</span>
                                        <span className="text-xs text-muted-foreground">
                                          {ex.sets} series · {ex.reps.join(', ')} reps · {Math.round(ex.volumeKg)} kg
                                        </span>
                                      </div>
                                    ))}
                                  </div>
                  ...
                  <TabsContent value="fuerza" className="mt-3">
                    <div className="space-y-4">
                      <WeeklyProgressCard sessions={sessions} logs={logs} exercises={exercises} />
                      <StrengthPanel exercises={exercises} getHistory={getExerciseHistory} />
                    </div>
                  </TabsContent>

                  <TabsContent value="workout" className="mt-3 space-y-4">
                    {!todayWorkout?.isWorkoutDay && !workoutStarted ? (
                      /* Non-workout day: show routine overview + planner */
                      <div className="space-y-4">
                        <Card className="p-4 text-center">
                          <p className="text-lg font-semibold">
                            {plannedDay ? `Rutina de ${DAY_LABELS[plannedDay]}` : "Hoy no es día de entrenamiento"}
                          </p>
                          <p className="text-sm text-muted-foreground mt-1">Revisa tu rutina, planifica un día o agrega ejercicios</p>
                          <Button variant="outline" size="sm" className="mt-3" onClick={() => { setTrainingType(trainingType); reload(trainingType); }}>
                            Cambiar rutina
                          </Button>
                        </Card>
                        ...
                        {trainingType === 'gimnasio' && <StrengthGoalsCard />}
                      </div>
                    ) : (
                        ...
                        {!workoutStarted ? (
                          <div className="space-y-4">
                            <Card className="p-6 text-center">
                              {trainingType === 'gimnasio' ? (
                                ...
                                <Box className="h-12 w-12 mx-auto text-teal-500 mb-3" />
                              )}
                          ...
                          <div className="space-y-4">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <span className="text-sm font-bold text-primary tabular-nums">{formatElapsed(elapsedSeconds)}</span>
                                ...
                                <div className="bg-gradient-to-r from-primary/10 to-primary/5 p-4">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      ...
                                      {trainingType === 'gimnasio' && hintFor(currentExerciseObj) && (
                                        <p className="text-[10px] text-muted-foreground mt-1.5">{hintFor(currentExerciseObj)}</p>
                                      )}
                                    </div>
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center gap-2 flex-1">
                                          ...
                                          <Input
                                            ...
                                            className={cn("h-8 text-center text-sm", trainingType === 'gimnasio' ? "w-16" : "w-20")}
                                            disabled={isCompleted}
                                          />
                                        ...
                                        {isCurrent && (
                                          <Button size="sm" onClick={() => completeSet(activeExercise, idx)} className="h-8 gap-1">
                            ...
                            {restTimer !== null && restTimer > 0 && (
                              <Card className="p-6 text-center bg-gradient-to-b from-primary/5 to-background">
                                ...
                                <p className="text-6xl font-bold text-primary tabular-nums">
                                  {String(Math.floor(restTimer / 60)).padStart(2, "0")}:
                            ...
                            {activeExercise < todayExercises.length - 1 && (
                              <Card className="p-3 opacity-60">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs text-muted-foreground">0/{todayExercises[activeExercise + 1].target_sets}</span>
                                </div>
                              </Card>
                            )}
