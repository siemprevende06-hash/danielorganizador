...
import { useWorkoutTracking, TrainingType } from "@/hooks/useWorkoutTracking";
import { Check, ChevronRight, Minus, Plus, Timer, Dumbbell, Play, SkipForward, Trophy, ChevronDown, ChevronUp, Box } from "lucide-react";
...
export default function Gym() {
  const {
    ...
    getTodayWorkout, getExercisesByDay, reload, DAY_NAMES, loadAllRoutines
  } = useWorkoutTracking();
      ...
      setSessionSets(
        todayExercises.map(ex =>
          Array.from({ length: ex.target_sets }, () => ({
            weight: trainingType === 'gimnasio' ? 0 : 0,
    ...
    if (allSetsComplete) {
      if (currentExerciseObj) {
        const avgWeight = trainingType === 'gimnasio'
          ? currentExSets.reduce((s, v) => s + v.weight, 0) / currentExSets.length
          : 0;
        logWorkout(
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2 bg-muted p-1 rounded-lg">
            <button
              onClick={() => { setTrainingType('gimnasio'); setWorkoutComplete(false); setWorkoutStarted(false); setActiveExercise(0); setSessionSets([]); }}
              className={cn(
            ...
            <button
              onClick={() => { setTrainingType('calistenia'); setWorkoutComplete(false); setWorkoutStarted(false); setActiveExercise(0); setSessionSets([]); }}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all",
              ...
              <Dialog open={setupOpen} onOpenChange={setSetupOpen}>
                <DialogTrigger asChild>
                  <Button size="lg" className="gap-2">
                    <Plus className="h-5 w-5" /> Crear Rutina de {trainingType === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>Nueva Rutina de {trainingType === 'gimnasio' ? 'Gimnasio' : 'Calistenia'}</DialogTitle></DialogHeader>
          ...
          <>
            {workoutComplete ? (
              <div className="max-w-lg mx-auto text-center space-y-6">
                ...
                <Button onClick={() => { setWorkoutComplete(false); setWorkoutStarted(false); setActiveExercise(0); setSessionSets([]); reload(trainingType); }}>
                  Volver al Inicio
                </Button>
              </div>
            ) : (
                                        ...
                                        <tr key={log.id} className="border-b border-muted/30 hover:bg-muted/20 transition-colors group">
                                          ...
                                          {trainingType === 'gimnasio' && (
                                            editingLog === log.id ? (
                                              ...
                                              <td className="text-center py-1 px-2 font-medium">{log.weight_kg || '—'} kg</td>
                                            )
                                          )}
                                          {editingLog === log.id ? (
                                            <>
