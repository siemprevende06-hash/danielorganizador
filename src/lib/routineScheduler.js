...
export function calculateMorningSchedule(wakeTime, fixedEndTime = "09:00", existingBlocks) {
    ...
    const morningBlocks = existingBlocks.filter(block => MORNING_BLOCK_IDS.includes(block.id));
    ...
    const adjustedBlocks = blocksToAdjust.map(block => {
...
export function applyMorningAdjustment(allBlocks, config) {
    const adjustedBlockMap = new Map(config.adjustedBlocks.map(block => [block.id, block]));
...
export function getTimeComparison(wakeTime) {
