...
export function calculateStreaks(completedDates) {
    ...
    for (let i = 0; i < completed.length - 1; i++) {
        ...
        const daysDiff = Math.round((currentDate.getTime() - nextDate.getTime()) / (1000 * 60 * 60 * 24));
        if (daysDiff === 1) {
            tempStreak++;
        }
        else {
...
export function getTodayCompletion(habitHistory, todayStr) {
    return habitHistory?.completedDates?.find((entry) => entry.date === todayStr);
}
export function getTodayDuration(habitHistory, todayStr) {
...
export function getMonthTotal(habitHistory) {
    if (!habitHistory?.completedDates)
        return 0;
