...
export function MusicTrendChart() {
    ...
    const data = useMemo(() => {
        ...
        if (scope === "week") {
            ...
            for (let i = 11; i >= 0; i--) {
                ...
                let minutes = 0;
                for (let d = 0; d < 7; d++)
                    minutes += byDay[dateKey(addDays(ws, d))] || 0;
    ...
    return (_jsxs("div", { className: "space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between gap-2 flex-wrap", children: [_jsxs("span", { className: "flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground", children: [_jsx(TrendingUp, { className: "w-3.5 h-3.5" }), " Tendencia de pr\u00E1ctica"] }), _jsx("div", { className: "flex rounded-lg border bg-muted/40 p-0.5", children: SCOPE_OPTIONS.map(opt => (_jsx("button", { onClick: () => setScope(opt.id), className: cn("px-2.5 py-1 text-[10px] font-medium rounded-md transition-colors", scope === opt.id ? "bg-background shadow-sm" : "text-muted-foreground hover:text-foreground"), children: opt.label }, opt.id))) })] }), total === 0 && !loading ? (_jsx("p", { className: "text-xs text-muted-foreground py-6 text-center", children: "A\u00FAn no hay pr\u00E1ctica registrada para mostrar la tendencia" })) : (_jsx(ResponsiveContainer, { width: "100%", height: 180, children: _jsxs(BarChart, { data: data, margin: { top: 4, right: 4, left: -20, bottom: 0 }, children: [_jsx(CartesianGrid, { strokeDasharray: "3 3", stroke: "hsl(var(--border))", vertical: false }), _jsx(XAxis, { dataKey: "label", tick: { fontSize: 9 }, interval: scope === "day" ? 1 : 0, tickLine: false, axisLine: false }), _jsx(YAxis, { allowDecimals: false, tick: { fontSize: 9 }, tickLine: false, axisLine: false }), _jsx(Tooltip, { cursor: { fill: "hsl(var(--muted))", opacity: 0.6 }, contentStyle: { fontSize: 11, borderRadius: 8 }, formatter: (value) => [`${value} min`, "Pr\u00E1ctica"] }), _jsx(Bar, { dataKey: "minutes", fill: "#ec4899", radius: [4, 4, 0, 0], maxBarSize: 28 })] }) }))] }));
}
