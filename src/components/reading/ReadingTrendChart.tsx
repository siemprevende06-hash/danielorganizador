  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive && data) setRows(data as PageRow[]);
      } catch {}
  ...
  const data = useMemo(() => {
    ...
    if (scope === "week") {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        let pages = 0;
        for (let d = 0; d < 7; d++) pages += byDay[dateKey(addDays(ws, d))] || 0;
    ...
    if (scope === "month") {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        let pages = 0;
        for (let d = ms; d < me; d = addDays(d, 1)) pages += byDay[dateKey(d)] || 0;
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      let pages = 0;
      for (let d = qs; d < qe; d = addDays(d, 1)) pages += byDay[dateKey(d)] || 0;
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(value: any) => [`${value} páginas`, "Leídas"]}
            />
            <Bar dataKey="pages" fill="#8b5cf6" radius={[4, 4, 0, 0]} maxBarSize={28} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
