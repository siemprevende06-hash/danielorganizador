...
function makeCards(rows: number): BoardCard[] {
...
interface Props {
  ...
  setSections: (next: BoardSection[]) => void;
  uploadFolder: string;
  emptyTitle: string;
  emptyAction: string;
  accent?: "vision" | "anti";
...
export function BoardSectionEditor({
  sections,
  setSections,
  uploadFolder,
  emptyTitle,
    ...
    Promise.allSettled(
      valid.map(async (url) => {
        ...
        try {
          ...
          // ignore fetch errors for offline cache
        }
      })
    );
  }, [sections]);
  ...
  const updateSection = (id: string, patch: Partial<BoardSection>) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {(selectedSection ? sections.filter((s) => s.id === selectedSection) : sections).map(
        (section) => (
          <Card
            key={section.id}
            className={cn(
              "p-4 md:p-5 space-y-4",
              accent === "anti" && "border-destructive/20"
            )}
          >
            ...
            <div className="grid grid-cols-3 gap-2 md:gap-3">
              {section.cards.map((card) => {
                ...
                return (
                  <div
                    key={card.id}
                    className="relative aspect-square rounded-xl overflow-hidden border-2 border-dashed bg-muted/30 group"
                  >
                    <input
