...
export function DailyMotivation() {
  ...
  const loadPersonalGoal = async () => {
    ...
    const { data: goal } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (goal) setPersonalGoal(goal.title);
  };
  ...
  return (
    <div className="bg-gradient-to-r from-amber-500/5 via-primary/5 to-purple-500/5 rounded-lg p-4 border border-border">
      ...
      <blockquote className="text-sm italic text-foreground mb-1 leading-relaxed">
        "{quote.text}"
      </blockquote>
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">— {quote.author}</p>
        <p className="text-[10px] text-muted-foreground/50">{index + 1}/{MOTIVATIONAL_QUOTES.length}</p>
      </div>
