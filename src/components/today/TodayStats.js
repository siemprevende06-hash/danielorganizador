...
export function TodayStats() {
    ...
    const loadStats = async () => {
        ...
        if (habitHistory) {
            habitHistory.forEach(h => {
                ...
                if (dates?.some((d) => d.date === today && d.status === 'completed')) {
                    habitsCompleted++;
                }
            });
        }
    ...
    return (_jsx("div", { className: "space-y-3", children: statItems.map((item, index) => (_jsxs("div", { className: "flex items-center justify-between py-2", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("div", { className: `p-2 rounded ${item.highlight ? 'bg-foreground text-background' : 'bg-muted'}`, children: _jsx(item.icon, { className: "w-4 h-4" }) }), _jsx("span", { className: "text-sm text-muted-foreground", children: item.label })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "font-mono font-medium", children: item.value }), item.percentage !== null && (_jsxs("span", { className: `text-xs px-1.5 py-0.5 rounded ${item.percentage >= 75 ? 'bg-success/20 text-success' :
                                item.percentage >= 50 ? 'bg-muted text-muted-foreground' :
                                    'bg-destructive/20 text-destructive'}`, children: [Math.round(item.percentage), "%"] }))] })] }, index))) }));
}
