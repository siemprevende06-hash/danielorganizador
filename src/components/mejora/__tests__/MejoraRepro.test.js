...
vi.mock('@/integrations/supabase/client', () => {
    ...
    const supabase = {
        from: vi.fn((table) => {
            if (table === 'daily_systems_tracking')
                return Promise.resolve({ data: tracking, error: null });
            if (table === 'daily_area_stats')
                return Promise.resolve({ data: dailyAreaStats, error: null });
            if (table === 'reading_sessions')
                return Promise.resolve({ data: [], error: null });
            if (table === 'reading_library')
                return Promise.resolve({ data: [], error: null });
            if (table === 'chess_sessions')
                return Promise.resolve({ data: [], error: null });
            if (table === 'music_practice_sessions')
                return Promise.resolve({ data: [], error: null });
            if (table === 'language_sessions')
                return Promise.resolve({ data: [], error: null });
            if (table === 'music_repertoire')
                return Promise.resolve({ data: [], error: null });
            return Promise.resolve({ data: [], error: null });
        }),
    };
...
describe('MejoraProcessPanel crash repro', () => {
    ...
    it('renders overview without crashing', async () => {
        render(_jsx(MejoraProcessPanel, { anchorDate: new Date('2026-08-05') }));
