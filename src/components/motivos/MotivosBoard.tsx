...
export default function MotivosBoard({
  storageKey,
  uploadFolder = "motivos",
  title = "Motivos",
  description = "Crea secciones con cuadrículas de imágenes desde tu galería",
  emptyTitle = "No hay secciones aún",
  emptyDescription = "Crea una sección para empezar a añadir imágenes.",
  newSectionLabel = "Nueva Sección",
}: MotivosBoardProps) {
  ...
  useEffect(() => {
    const valid = sections
      .flatMap((s) => s.cards.map((c) => c.image_url))
  ...
  const addSection = () => {
    ...
    persist([
      ...sections,
      { id: uid(), name: `Sección ${sections.length + 1}`, rows, cards: makeCards(rows) },
    ]);
  };

  const updateSection = (sectionId: string, patch: Partial<MotivoSection>) => {
    persist(sections.map((s) => (s.id === sectionId ? { ...s, ...patch } : s)));
  };
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-3">
            {section.cards.map((card) => {
              ...
              return (
                <div
                  key={card.id}
                  className="relative aspect-square rounded-lg border-2 border-dashed bg-muted/30 overflow-hidden group"
                >
                  <input
                    ...
                    <>
                      ...
                      <div className="absolute top-2 right-2 flex gap-1">
                        <button
                          ...
                          className="h-7 w-7 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-black/70 transition-colors"
