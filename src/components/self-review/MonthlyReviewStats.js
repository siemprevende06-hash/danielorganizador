...
export function MonthlyReviewStats({ monthDate }) {
    ...
    if (!data) {
        return (_jsx(Card, { className: "p-6 text-center", children: _jsx("p", { className: "text-muted-foreground", children: "No hay datos para este mes" }) }));
    }
                            ...
                            return (_jsx("div", { className: "grid grid-cols-7 gap-1.5", children: weekDays.map(day => {
                                ...
                                .map(([habitId, count]) => (_jsxs("span", { className: cn("text-[9px] px-2 py-0.5 rounded-full font-medium border", count >= data.totalDays * 0.7 ? "bg-green-500/15 text-green-600 border-green-500/30" :
                                    count >= data.totalDays * 0.4 ? "bg-amber-500/15 text-amber-600 border-amber-500/30" :
                                        "bg-muted text-muted-foreground border-transparent"), children: [habitId.replace(/-/g, ' '), " ", count, "/", data.totalDays] }, habitId))), Object.keys(data.habitCompletions).length === 0 && (_jsx("p", { className: "text-xs text-muted-foreground", children: "No hay h\u00E1bitos registrados este mes" }))] })] })] }));
}
