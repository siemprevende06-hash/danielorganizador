...
export function MonthlyReviewStats({ monthDate }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Day by Day Breakdown - grouped by week */}
      <div>
        ...
        <div className="space-y-1">
          {Array.from({ length: Math.ceil(data.dayDetails.length / 7) }).map((_, weekIdx) => {
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {Object.keys(data.habitCompletions).length === 0 && (
            <p className="text-xs text-muted-foreground">No hay hábitos registrados este mes</p>
          )}
        </div>
      </div>
    </div>
