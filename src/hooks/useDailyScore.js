...
function computeScore(sysRes, routineRes, miniRes) {
    const completions = sysRes?.completions || {};
    const timeData = sysRes?.time_data || {};
    const miniDefs = [];
    const raw = miniRes?.setting_value;
...
export function useDailyScore() {
    ...
    const refresh = useCallback(async () => {
        ...
        if (!online) {
            const cached = await getCached("daily_score", CACHE_KEY);
            if (cached) {
