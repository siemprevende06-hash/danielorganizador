...
export function useProjects() {
    ...
    const migrateFromLegacy = useCallback(async () => {
        try {
            ...
            const { error } = await supabase.from('projects').insert(migrated);
            if (error)
                throw error;
    ...
    const persistOne = useCallback(async (project) => {
    ...
    const addTask = useCallback((projectId, title, dueDate) => {
        const task = {
            id: crypto.randomUUID(),
            title,
            completed: false,
        ...
        setProjects(prev => {
            const updated = prev.map(p => p.id === projectId ? { ...p, tasks: [...p.tasks, task] } : p);
    ...
    const addSubTask = useCallback((projectId, taskId, title) => {
    ...
    const toggleSubTask = useCallback((projectId, taskId, subTaskId) => {
        setProjects(prev => {
            const updated = prev.map(p => p.id === projectId ? {
                ...p,
                tasks: p.tasks.map(t => t.id === taskId ? {
