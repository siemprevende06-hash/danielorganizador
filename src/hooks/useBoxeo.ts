...
import type { BoxeoTecnica, BoxeoSesion } from '@/lib/definitions';
import { toast } from 'sonner';

const TECNICAS_POR_DEFECTO: Omit<BoxeoTecnica, 'id'>[] = [
...
export function useBoxeo() {
  const [tecnicas, setTecnicas] = useState<BoxeoTecnica[]>([]);
  const [sesiones, setSesiones] = useState<BoxeoSesion[]>([]);
  ...
  const seedTecnicas = async () => {
    const { error } = await supabase.from('boxeo_tecnicas').insert(
      TECNICAS_POR_DEFECTO.map(t => ({ ...t }))
    );
    if (error) { toast.error('Error al cargar técnicas'); return; }
    toast.success('Técnicas de boxeo cargadas');
    loadData();
  };

  const actualizarDominio = async (id: string, nivel: number) => {
  ...
  const agregarSesion = async (sesion: Omit<BoxeoSesion, 'id'>) => {
