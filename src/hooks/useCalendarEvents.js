...
export function useCalendarEvents(month) {
    ...
    const loadEvents = useCallback(async () => {
        ...
        setEvents((data || []).map(normalizeCalendarEvent));
        setLoading(false);
    }, [month]);
    useEffect(() => {
        setLoading(true);
    ...
    const addEvent = async (title, eventDate, category, description, startTime, endTime) => {
    ...
    const getEventsForDay = (dateStr) => events.filter(e => e.event_date === dateStr);
