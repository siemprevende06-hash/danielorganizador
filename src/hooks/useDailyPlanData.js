...
export function useDailyPlanData(date) {
    ...
    const [planAssignments, setPlanAssignments] = useState(null);
    const [planRoutineType, setPlanRoutineType] = useState(null);
    const [planLanguage, setPlanLanguage] = useState(null);
    ...
    const loadPlanForDate = async () => {
        ...
        try {
            ...
            if (plan && plan.block_assignments) {
                setPlanAssignments(plan.block_assignments);
        ...
        if (planAssignments && dateStr === todayStr) {
            ...
            tasks.forEach(task => {
                if (task.routine_block_id) {
                    if (!grouped[task.routine_block_id])
                        grouped[task.routine_block_id] = [];
    ...
    const assignTaskToBlock = useCallback(async (taskId, blockId) => {
        if (planAssignments) {
            ...
            if (dateStr === todayStr) {
                ...
                try {
                    await supabase.from(table).update({ routine_block_id: blockId }).eq('id', taskId);
                }
                catch (error) {
    ...
    const removeTaskFromBlock = useCallback(async (taskId) => {
        if (planAssignments) {
            setPlanAssignments(prev => {
                const next = { ...prev };
                for (const bId of Object.keys(next)) {
            ...
            if (dateStr === todayStr) {
                ...
                const table = task.source === 'entrepreneurship' ? 'entrepreneurship_tasks' : 'tasks';
                try {
                    await supabase.from(table).update({ routine_block_id: null }).eq('id', taskId);
                }
                catch (error) {
    ...
    const toggleTaskDone = useCallback(async (taskId) => {
        const task = tasks.find(t => t.id === taskId);
        if (!task)
            return;
        const table = task.source === 'entrepreneurship' ? 'entrepreneurship_tasks' : 'tasks';
        ...
        try {
            await supabase.from(table).update({ completed: next }).eq('id', taskId);
