...
export function useAchievements() {
  ...
  const unlock = useCallback(async (key: string) => {
    ...
    if (!def) return;
    if (achievements.some(a => a.achievement_key === key)) return;
  ...
  const checkAchievements = useCallback(async () => {
    ...
    if (taskCount && taskCount >= 1) await unlock('first_task');
    if (taskCount && taskCount >= 10) await unlock('tasks_10');
    if (taskCount && taskCount >= 50) await unlock('tasks_50');
    if (taskCount && taskCount >= 100) await unlock('tasks_100');
    ...
    if (maxStreak >= 3) await unlock('streak_3');
    if (maxStreak >= 7) await unlock('streak_7');
    if (maxStreak >= 30) await unlock('streak_30');
    ...
    if (totalFocus >= 60) await unlock('focus_60');
    if (totalFocus >= 300) await unlock('focus_300');

    // Check books
    const { count: booksCount } = await supabase.from('reading_library').select('*', { count: 'exact', head: true }).eq('status', 'read');
    if (booksCount && booksCount >= 1) await unlock('books_1');
    if (booksCount && booksCount >= 5) await unlock('books_5');
  }, [unlock]);
