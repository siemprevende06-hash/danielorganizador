...
export const AREA_ORDER = ['universidad', 'emprendimiento', 'proyectos', 'lectura', 'musica', 'ajedrez', 'game', 'idiomas', 'gym', 'general'];
const AREA_DISPLAY = {
...
export function emptyAreaResult() {
    const byArea = {};
...
const dayLabel = (d) => format(new Date(`${d}T12:00:00`), 'EEE d', { locale: es });
export function useResultadosPeriodo(start, end) {
    const startStr = format(start, 'yyyy-MM-dd');
    const endStr = format(end, 'yyyy-MM-dd');
    return useQuery({
        queryKey: ['resultados', startStr, endStr],
        queryFn: async () => {
            const [tasksRes, entTasksRes, areaStatsRes, systemsRes, reviewsRes, readRes, musicRes, chessRes, logsRes, focusRes, citasRes, intimidadRes, eventosRes, ingresoRes, libraryRes, repertoireRes, projectsRes, subjectsRes, entregasRes, examsRes, partialsRes, topicRes, goalsRes, entPendingRes, subjPendingRes,] = await Promise.all([
            ...
            const entMap = new Map((entregasRes.data || []).map((e) => [e.id, e.name]));
            const entTaskIds = new Set([
                ...(entTasksRes.data || []).map((t) => t.id),
                ...(entPendingRes.data || []).map((t) => t.id),
            ]);
            ...
            const tasks = [
                ...(tasksRes.data || []).filter((t) => !(t.source === 'entrepreneurship' && entTaskIds.has(t.source_id))),
                ...(entTasksRes.data || []).map((t) => ({ ...t, source: 'emprendimiento', area_id: 'emprendimiento', source_id: t.entrepreneurship_id, _ent: true })),
            ].map((t) => ({
                ...t,
            ...
            const subjectTaskMap = {};
            const mergeSubjectTask = (subjId, t) => {
                if (!activeSubjects.includes(subjId))
                    return;
            ...
            (tasksRes.data || []).forEach((t) => {
                if (t.source === 'university' && t.source_id)
                    mergeSubjectTask(t.source_id, t);
            });
            (subjPendingRes.data || []).forEach((t) => {
                if (t.source === 'university' && t.source_id)
                    mergeSubjectTask(t.source_id, t);
            });
            const examsRows = (examsRes.data || []).filter((e) => activeSubjects.includes(e.subject_id));
            const partialsRows = (partialsRes.data || []).filter((p) => activeSubjects.includes(p.subject_id));
            const topicRows = (topicRes.data || []).filter((t) => activeSubjects.includes(t.subject_id));
            const universitySubjects = activeSubjects.map(id => {
            ...
            const uniOther = tasks.filter((t) => normalizeArea(t.area_id || t.source || t.area) === 'universidad' && t.source !== 'university');
            // --- Emprendimiento: negocios activos ---
            const businessTaskMap = {};
            const mergeBusinessTask = (bizId, t) => {
                if (!activeBusinesses.includes(bizId))
                    return;
                const list = (businessTaskMap[bizId] = businessTaskMap[bizId] || []);
                if (!list.some(x => x.id === t.id))
                    list.push(t);
            };
            (entTasksRes.data || []).forEach((t) => mergeBusinessTask(t.entrepreneurship_id, t));
            (entPendingRes.data || []).forEach((t) => mergeBusinessTask(t.entrepreneurship_id, t));
            const goalRows = (goalsRes.data || []).filter((g) => activeBusinesses.includes(g.entrepreneurship_id));
            const businessList = activeBusinesses.map(id => {
            ...
            const entOther = tasks.filter((t) => !t._ent && normalizeArea(t.area_id || t.source || t.area) === 'emprendimiento');
            // --- Proyectos: proyectos activos ---
            const projectList = (projectsRes.data || []).filter((p) => activeProjects.includes(p.id)).map((p) => {
                const pts = (p.tasks || []);
            ...
            const projOther = tasks.filter((t) => normalizeArea(t.area_id || t.source || t.area) === 'proyectos');
            const { quarter: planQ, year: planY } = getQuarterFromDate(start);
            const plan = loadTrimestralPlanFromLocal(`Q${planQ}_${planY}`);
            const monthKey = `month${start.getMonth() - (planQ - 1) * 3 + 1}`;
            const planBookIds = new Set();
            const planSongIds = new Set();
            if (plan) {
                if (end.getMonth() === start.getMonth()) {
                    const dist = plan.distribution?.[monthKey];
                    (dist?.books || plan.books?.selected || []).forEach((id) => planBookIds.add(id));
                    (dist?.songs || plan.songs?.selected || []).forEach((id) => planSongIds.add(id));
                }
                else {
                    ['month1', 'month2', 'month3'].forEach(mk => {
                        const dist = plan.distribution?.[mk];
                        (dist?.books || []).forEach((id) => planBookIds.add(id));
                        (dist?.songs || []).forEach((id) => planSongIds.add(id));
                    });
                }
            }
            ...
            const songsPlan = [...planSongIds]
                .map(id => repertoire.find((s) => s.id === id))
                .filter(Boolean)
                .map((s) => ({
                id: s.id,
                title: s.title,
            ...
            const systemsDone = systems.reduce((acc, s) => acc + Object.values(s.completions || {}).filter(v => v === true).length, 0);
            const systemsTotal = systems.length > 0 ? Math.max(...systems.map((s) => Object.keys(s.completions || {}).length)) : 0;
            const systemsMin = systems.reduce((a, s) => a + Object.values(s.time_data || {}).reduce((x, v) => x + (Number(v) || 0), 0), 0);
            const workoutMin = systems.reduce((a, s) => a + (s.workout_duration || 0), 0);
            const focusMin = focus.reduce((a, f) => a + (f.duration_minutes || 0), 0);
            let pages = 0;
            let pagesGoal = 0;
            let readMin = 0;
            const readPerDay = {};
            ...
            areaStats.forEach((s) => {
                if (normalizeArea(s.area_id) === 'lectura')
                    pagesGoal += s.pages_goal || 0;
            });
            let musicMin = 0;
            const musicPerDay = {};
            const songs = new Set();
            ...
            logs.forEach((l) => {
                ...
                if (kg > 0 && (gymMax === null || kg > gymMax))
                    gymMax = kg;
                const d = l.log_date;
                gymPerDay[d] = Math.max(gymPerDay[d] || 0, kg);
            });
            const ingAmount = ingreso.reduce((a, i) => a + (Number(i.amount) || 0), 0);
            const allDates = new Set();
            tasks.forEach((t) => t.due_date && allDates.add(format(new Date(t.due_date), 'yyyy-MM-dd')));
            systems.forEach((s) => allDates.add(s.tracking_date));
            areaStats.forEach((s) => allDates.add(s.stat_date));
            reviews.forEach((r) => allDates.add(r.review_date));
            readings.forEach((r) => allDates.add(r.session_date));
            music.forEach((m) => allDates.add(m.practice_date));
            chess.forEach((c) => allDates.add(c.session_date));
            logs.forEach((l) => allDates.add(l.log_date));
            const perDay = [...allDates].sort().map(dt => {
                const dayTasks = tasks.filter((t) => t.due_date && format(new Date(t.due_date), 'yyyy-MM-dd') === dt);
                ...
                if (review?.overall_rating)
                    score = review.overall_rating * 20;
                else if (dayTasks.length > 0)
                    score = Math.round((dayTasks.filter((t) => t.completed).length / dayTasks.length) * 100);
                else if (spentMin > 0)
                    score = 50;
            ...
            return {
                ...
                globalDone: tasks.filter((t) => t.completed).length,
                ...
                reviews: {
                    ...
                    avgRating: reviews.length ? reviews.reduce((a, r) => a + (r.overall_rating || 0), 0) / reviews.length : 0,
                },
                lectura: {
                    pages,
                    pagesGoal,
