...
const makeBlock = (id, title, startTime, endTime, order, isFocusBlock = false, tasks, defaultFocus) => ({
    id,
    title,
    startTime,
    endTime,
...
export const useRoutineBlocks = () => {
    const [routineType, setRoutineTypeState] = useState('disciplina');
    const [blocks, setBlocks] = useState([]);
    const [isLoaded, setIsLoaded] = useState(false);
    useEffect(() => {
        const savedType = localStorage.getItem(ROUTINE_TYPE_KEY);
    ...
    const setRoutineType = useCallback((type) => {
        setRoutineTypeState(type);
    ...
    const updateBlockFocus = useCallback((blockId, focus) => {
        const newBlocks = blocks.map(block => block.id === blockId ? { ...block, currentFocus: focus } : block);
        saveBlocks(newBlocks);
    }, [blocks, saveBlocks]);
    return {
        blocks,
