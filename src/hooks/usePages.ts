...
export function usePageList() {
  ...
  const createPage = useCallback((overrides?: Partial<PageMeta>): PageMeta => {
  ...
  const updatePage = useCallback((id: string, updates: Partial<PageMeta>) => {
    const meta = getPagesMeta()
...
export function usePageContent(pageId: string | undefined) {
  ...
  useEffect(() => {
    if (!pageId) { setBlocks([]); setLoading(false); return }
    setLoading(true)
  ...
  const updateBlocks = useCallback((newBlocks: Block[]) => {
    ...
    timerRef.current = setTimeout(() => {
      if (pageId) savePageContent(pageId, newBlocks)
    }, 500)
  }, [pageId])

  const saveNow = useCallback(() => {
    if (pageId) savePageContent(pageId, blocks)
  }, [pageId, blocks])
