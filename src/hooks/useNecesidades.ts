...
const STATIC_NEEDS: (Omit<Necesidad, 'id' | 'created_at'>)[] = [
...
export function useNecesidades() {
    ...
    try {
      ...
      if (error) {
        ...
        const { data: inserted } = await supabase
          ...
          .select()
          .order('orden');
        if (inserted) setNecesidades(inserted);
        else setNecesidades(STATIC_NEEDS.map((n, i) => ({ ...n, id: `static-${i}` })));
      }
    } catch (err) {
  ...
  const actualizarProgreso = async (necesidadId: string, progreso: number) => {
    const clamped = Math.max(0, Math.min(100, progreso));
    setNecesidades(prev =>
      prev.map(n => n.necesidad_id === necesidadId ? { ...n, progreso: clamped } : n)
    );
    await supabase
      .from('necesidades')
  ...
  const getProgresoGeneral = () => {
    if (necesidades.length === 0) return 0;
    return Math.round(necesidades.reduce((s, n) => s + (n.progreso || 0), 0) / necesidades.length);
  };

  const getNecesidadByArea = (area: string) =>
    necesidades.find(n => n.area_referencia === area);
