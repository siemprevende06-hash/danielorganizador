...
export function getPeriodRange(type, referenceDate) {
    switch (type) {
        case 'weekly':
...
export function usePeriodicReview(type, referenceDate) {
        ...
        try {
            ...
            if (data) {
                setReview({
                    ...
                    effort_objectives: data.effort_objectives || [],
                    result_objectives: data.result_objectives || [],
    ...
    const loadConsistency = useCallback(async () => {
        try {
            ...
            const snapshots = areas.map(area => {
                ...
                const uniqueDaysFromFocus = new Set(areaSessions.map(s => s.start_time?.split('T')[0]));
                // Count unique days from daily_area_stats
                const areaStatEntries = areaStats.filter(s => s.area_id === area.id && s.completed);
                const uniqueDaysFromStats = new Set(areaStatEntries.map(s => s.stat_date));
                // Merge unique days
    ...
    const addObjective = (objectiveType, objective) => {
        if (!review)
            return;
        const newObj = { ...objective, id: crypto.randomUUID() };
        const key = objectiveType === 'effort' ? 'effort_objectives' : 'result_objectives';
        setReview(prev => prev ? { ...prev, [key]: [...prev[key], newObj] } : prev);
    };
    const updateObjective = (objectiveType, id, updates) => {
        if (!review)
            return;
        const key = objectiveType === 'effort' ? 'effort_objectives' : 'result_objectives';
        setReview(prev => prev ? {
    ...
    const removeObjective = (objectiveType, id) => {
        if (!review)
            return;
        const key = objectiveType === 'effort' ? 'effort_objectives' : 'result_objectives';
        setReview(prev => prev ? {
