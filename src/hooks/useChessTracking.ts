...
export interface ChessGoals {
  id: string;
  target_elo: number;
  target_games_per_month: number;
...
export const useChessTracking = () => {
  ...
  const addSession = async (s: Partial<ChessSession>) => {
    ...
    if (!error && data) setSessions(prev => [data as ChessSession, ...prev]);
    return { data, error };
  };

  const upsertGoals = async (g: Partial<ChessGoals>) => {
    if (goals) {
      const { error } = await supabase.from("chess_goals").update(g).eq("id", goals.id);
      if (!error) setGoals({ ...goals, ...g } as ChessGoals);
      return { error };
    } else {
      ...
      if (!error && data) setGoals(data as ChessGoals);
      return { error };
    }
  };
