...
export function useDailyReview(date) {
    ...
    const [systemsTracking, setSystemsTracking] = useState(null);
    ...
    const loadSystemsTracking = async () => {
        ...
        if (data) {
            setSystemsTracking({
                ...
                waterData: data.water_data || {},
                workAssignments: data.work_assignments || {},
                blockCompletions: data.block_completions || {},
    ...
    const loadReview = async () => {
        ...
        if (existingReview) {
            setReview({
                ...
                blockRatings: existingReview.block_ratings || [],
    ...
    const calculateDayStats = async () => {
        ...
        habitHistory?.forEach(h => {
            ...
            if (dates?.some((d) => d.date === reviewDate && d.status === 'completed')) {
                habitsCompleted++;
            }
        });
    ...
    const saveReview = async (updatedReview) => {
        ...
        const dataToSave = {
            ...
            block_ratings: (updatedReview.blockRatings ?? review.blockRatings),
    ...
    const updateBlockRating = (blockId, rating, notes, status) => {
        if (!review)
            return;
