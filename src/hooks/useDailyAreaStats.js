...
export const useDailyAreaStats = () => {
    ...
    const getOrCreateStat = useCallback(async (areaId) => {
    ...
    const toggleCompletion = useCallback(async (areaId) => {
    ...
    // Update time spent
    const updateTimeSpent = useCallback(async (areaId, minutes) => {
        let stat = stats[areaId];
        if (!stat) {
            stat = await getOrCreateStat(areaId);
            if (!stat)
    ...
    // Update time goal
    const updateTimeGoal = useCallback(async (areaId, minutes) => {
        let stat = stats[areaId];
        if (!stat) {
            stat = await getOrCreateStat(areaId);
            if (!stat)
    ...
    // Update pages done
    const updatePagesDone = useCallback(async (areaId, pages) => {
        let stat = stats[areaId];
        if (!stat) {
            stat = await getOrCreateStat(areaId);
            if (!stat)
    ...
    // Update pages goal
    const updatePagesGoal = useCallback(async (areaId, pages) => {
        let stat = stats[areaId];
        if (!stat) {
            stat = await getOrCreateStat(areaId);
            if (!stat)
    ...
    const addTime = useCallback(async (areaId, minutes) => {
        let stat = stats[areaId];
        if (!stat) {
            stat = await getOrCreateStat(areaId);
            if (!stat)
