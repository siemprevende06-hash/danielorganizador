...
export const usePerformanceModes = () => {
    ...
    const saveModes = useCallback((newModes, affectedModeId) => {
    ...
    const updateMode = useCallback((modeId, updates) => {
        const newModes = modes.map(mode => mode.id === modeId ? { ...mode, ...updates } : mode);
        saveModes(newModes, modeId);
    }, [modes, saveModes]);
    // Update a block within a mode (no validation for overlaps)
    const updateBlockInMode = useCallback((modeId, blockId, updates) => {
        const newModes = modes.map(mode => {
            if (mode.id === modeId) {
                return {
                    ...mode,
                    blocks: mode.blocks.map(block => block.id === blockId ? { ...block, ...updates } : block),
                };
            }
            return mode;
        });
    ...
    const addBlockToMode = useCallback((modeId, block) => {
        const newModes = modes.map(mode => {
            if (mode.id === modeId) {
                return {
                    ...mode,
    ...
    const removeBlockFromMode = useCallback((modeId, blockId) => {
        const newModes = modes.map(mode => {
            if (mode.id === modeId) {
                return {
                    ...mode,
    ...
    const resetModeToDefault = useCallback((modeId) => {
