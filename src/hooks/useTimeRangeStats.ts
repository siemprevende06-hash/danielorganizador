...
import { format, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfQuarter, endOfQuarter, subWeeks, subMonths } from 'date-fns';
...
interface RawRow {
  ...
  time_data: Record<string, number> | null;
  water_data: Record<string, boolean> | null;
  block_completions: Record<string, boolean> | null;
  workout_duration: number | null;
  wake_time: string | null;
  sleep_time: string | null;
}

function parseDay(row: RawRow): DayStats {
  const completions = (row.completions || {}) as Record<string, boolean>;
  const timeData = (row.time_data || {}) as Record<string, number>;
...
export function useTimeRangeStats() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // TODAY
      let dayStats: DayStats | null = null;
      if (currentRow.data) {
        dayStats = parseDay(currentRow.data as RawRow);
      }

      // WEEK
      const weekDays = (weekRows.data || []).map(r => parseDay(r as RawRow));
      ...
      // PREV WEEK
      const prevWeekDays = (prevWeekRows.data || []).map(r => parseDay(r as RawRow));
      ...
      if (weekAvgCompletion > prevWeekAvg + 5) trend = 'up';
      else if (weekAvgCompletion < prevWeekAvg - 5) trend = 'down';

      // MONTH
      const monthDays = (monthRows.data || []).map(r => parseDay(r as RawRow));
      ...
      // QUARTER
      const quarterDays = (quarterRows.data || []).map(r => parseDay(r as RawRow));
