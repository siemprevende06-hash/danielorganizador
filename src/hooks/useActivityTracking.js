...
export const useActivityTracking = () => {
    ...
    const getActivity = useCallback((activityType) => {
        return activities.find(a => a.activity_type === activityType);
    }, [activities]);
    const markComplete = useCallback(async (activityType, customDuration) => {
    ...
    const addBonusTime = useCallback(async (activityType, bonusMinutes) => {
        const existingActivity = getActivity(activityType);
        if (!existingActivity) {
    ...
    const unmarkActivity = useCallback(async (activityType) => {
        const existingActivity = getActivity(activityType);
        if (!existingActivity)
            return;
        try {
    ...
    const getActivityStatus = useCallback((activityType) => {
    ...
    const getTotalMinutes = useCallback((activityType) => {
        const activity = getActivity(activityType);
        if (!activity)
            return 0;
