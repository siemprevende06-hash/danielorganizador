...
export function useEffortResultStats(dateStr) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [areaStats, areaStreaks, areaGoalsConfig, tasks, weekAreaStats] = await Promise.all([
                ...
                safeQuery(supabase.from('tasks').select('id, completed, area_id, status')
                    .gte('due_date', `${today}T00:00:00`).lte('due_date', `${today}T23:59:59`)),
                safeQuery(supabase.from('daily_area_stats').select('area_id, stat_date, completed')
                    .gte('stat_date', weekStart).lte('stat_date', weekEnd)),
            ]);
            let allTasks = tasks || [];
            if (allTasks.length === 0) {
                const localData = getLocalTasksForDate(today);
                allTasks = localData.tasks.map((t) => ({
                    id: t.id,
            ...
            const allAreas = AREAS.map(area => {
                ...
                const daysActiveThisWeek = new Set((weekAreaStats || []).filter((s) => s.area_id === area.id && s.completed).map((s) => s.stat_date)).size;
